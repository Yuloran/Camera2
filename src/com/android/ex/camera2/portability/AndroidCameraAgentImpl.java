...
class AndroidCameraAgentImpl extends CameraAgent
{
    ...
    private CameraExceptionCallback mCameraExceptionCallback = new CameraExceptionCallback()
    {
        @Override
    ...
    @Override
    public void setCameraDefaultExceptionCallback(CameraExceptionCallback callback, Handler handler)
    {
    ...
    private static class AndroidCameraDeviceInfo implements CameraDeviceInfo
    {
        ...
        private AndroidCameraDeviceInfo(Camera.CameraInfo[] info, int numberOfCameras, int firstBackCameraId, int
                firstFrontCameraId)
        {
    ...
    private class CameraHandler extends HistoryHandler
    {
        ...
        private class CaptureCallbacks
        {
            ...
            CaptureCallbacks(ShutterCallback shutter, PictureCallback raw, PictureCallback postView, PictureCallback
                    jpeg)
            {
                mShutter = shutter;
                mRaw = raw;
        ...
        public void requestTakePicture(final ShutterCallback shutter, final PictureCallback raw, final
        PictureCallback postView, final PictureCallback jpeg)
        {
                ...
                switch (msg.what)
                {
                    case CameraActions.OPEN_CAMERA:
                    {
                        ...
                        if (mCamera != null)
                        {
                            ...
                            {
                                openCallback.onCameraOpened(new AndroidCameraProxyImpl(cameraId, mCamera,
                    ...
                    case CameraActions.RECONNECT:
                    {
                        ...
                        } catch (IOException ex)
                        {
                            if (cbForward != null)
                            {
                                cbForward.onReconnectionFailure(AndroidCameraAgentImpl.this, generateHistoryString
                                        (mCameraId));
                            }
                            break;
                        }
                        ...
                        if (cbForward != null)
                        {
                            cbForward.onCameraOpened(new AndroidCameraProxyImpl(cameraId, mCamera, mCharacteristics,
                                    mCapabilities));
                        }
                        break;
                    }
                ...
                } else
                {
                    if (mCamera == null)
                    {
                        if (msg.what == CameraActions.OPEN_CAMERA)
                        {
                            ...
                            if (msg.obj != null)
                            {
                                ((CameraOpenCallback) msg.obj).onDeviceOpenFailure(msg.arg1, generateHistoryString
                                        (cameraId));
                            }
                        } else
                        {
    ...
    private class AndroidCameraProxyImpl extends CameraAgent.CameraProxy
    {
        ...
        private AndroidCameraProxyImpl(int cameraId, Camera camera, CameraDeviceInfo.Characteristics characteristics,
        ...
        public void setPreviewDataCallback(final Handler handler, final CameraPreviewDataCallback cb)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_PREVIEW_CALLBACK, PreviewCallbackForward
        ...
        public void setOneShotPreviewCallback(final Handler handler, final CameraPreviewDataCallback cb)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_ONE_SHOT_PREVIEW_CALLBACK, PreviewCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, cb))
                            .sendToTarget();
                }
            });
            ...
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_PREVIEW_CALLBACK_WITH_BUFFER,
                            PreviewCallbackForward.getNewInstance(handler, AndroidCameraProxyImpl.this, cb))
                            .sendToTarget();
                }
            });
        }
        ...
        public void autoFocus(final Handler handler, final CameraAFCallback cb)
        {
            ...
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    mCameraHandler.obtainMessage(CameraActions.AUTO_FOCUS, afCallback).sendToTarget();
                }
            });
        }
        ...
        public void setAutoFocusMoveCallback(final Handler handler, final CameraAFMoveCallback cb)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_AUTO_FOCUS_MOVE_CALLBACK, AFMoveCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, cb))
                            .sendToTarget();
                }
            });
        ...
        @Override
        public void takePicture(final Handler handler, final CameraShutterCallback shutter, final
        CameraPictureCallback raw, final CameraPictureCallback post, final CameraPictureCallback jpeg)
        {
            ...
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraState.waitForStates(AndroidCameraStateHolder.CAMERA_IDLE | AndroidCameraStateHolder
                            .CAMERA_UNLOCKED);
                    mCameraHandler.requestTakePicture(ShutterCallbackForward.getNewInstance(handler,
                            AndroidCameraProxyImpl.this, shutter), PictureCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, raw), PictureCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, post), jpegCallback);
                }
            });
        }
        ...
        public void setZoomChangeListener(final OnZoomChangeListener listener)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_ZOOM_CHANGE_LISTENER, listener).sendToTarget();
                }
            });
        }
        ...
        public void setFaceDetectionCallback(final Handler handler, final CameraFaceDetectionCallback cb)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_FACE_DETECTION_LISTENER, FaceDetectionCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, cb)).sendToTarget();
                }
            });
        }
        ...
        public void setErrorCallback(final Handler handler, final CameraErrorCallback cb)
        {
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.SET_ERROR_CALLBACK, ErrorCallbackForward
                            .getNewInstance(handler, AndroidCameraProxyImpl.this, cb))
                            .sendToTarget();
                }
            });
        ...
        public void setParameters(final Parameters params)
        {
            ...
            mDispatchThread.runJob(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraState.waitForStates(AndroidCameraStateHolder.CAMERA_IDLE | AndroidCameraStateHolder
                            .CAMERA_UNLOCKED);
                    mCameraHandler.obtainMessage(CameraActions.SET_PARAMETERS, flattenedParameters).sendToTarget();
                }
            });
        }
        ...
        public Parameters getParameters()
        {
            ...
            mDispatchThread.runJobSync(new Runnable()
            {
                ...
                public void run()
                {
                    mCameraHandler.obtainMessage(CameraActions.GET_PARAMETERS, parametersHolder).sendToTarget();
        ...
        {
            return applySettingsHelper(settings, AndroidCameraStateHolder.CAMERA_IDLE | AndroidCameraStateHolder
                    .CAMERA_UNLOCKED);
        }
    ...
    private static class ErrorCallbackForward implements Camera.ErrorCallback
    {
         ...
         * or null if any parameter is null.
         */
        public static ErrorCallbackForward getNewInstance(Handler handler, CameraProxy camera, CameraErrorCallback cb)
        {
            if (handler == null || camera == null || cb == null)
            {
                return null;
        ...
        private ErrorCallbackForward(Handler h, CameraProxy camera, CameraErrorCallback cb)
        {
            mHandler = h;
            mCamera = camera;
            mCallback = cb;
    ...
    private static class AFMoveCallbackForward implements AutoFocusMoveCallback
    {
        ...
        @Override
        public void onAutoFocusMoving(final boolean moving, android.hardware.Camera camera)
        {
            mHandler.post(new Runnable()
            {
                @Override
    ...
    private static class ShutterCallbackForward implements ShutterCallback
    {
         ...
         * or null if any parameter is null.
         */
        public static ShutterCallbackForward getNewInstance(Handler handler, CameraProxy camera,
    ...
    private static class PictureCallbackForward implements PictureCallback
    {
         ...
         */
        public static PictureCallbackForward getNewInstance(Handler handler, CameraProxy camera,
    ...
    private static class PreviewCallbackForward implements PreviewCallback
    {
         ...
         * or null if any parameters is null.
         */
        public static PreviewCallbackForward getNewInstance(Handler handler, CameraProxy camera,
    ...
    private static class FaceDetectionCallbackForward implements FaceDetectionListener
    {
         ...
         * or null if any parameter is null.
         */
        public static FaceDetectionCallbackForward getNewInstance(Handler handler, CameraProxy camera,
